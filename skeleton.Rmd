---
title: "Paddy Rice Crops Classification."
subtitle: "A Time-Weighted Dynamic Time Warping Approach"
...
```{r capa, fig.fullwidth = TRUE, fig.width=30, fig.height=10, fig.cap="agriculture fields in southern Brazil.", echo=FALSE}
knitr::include_graphics("Plots/sits_capa.png")
```
...
I stumbled upon Dynamic Time Warping (DTW) during a lecture in Earth Observation, where later I got to do a case of study of it. 

There is no much things of TW-DTW for Remote Sensing (RS) around it. Whether most of the content are on __trending mode__^[okay, fair!!] towards DL^[Deep Learning/Machine Learning] approaches, than compared with old-fashioned way of time-series^[Satellite Image Time Series] analysis. This give me the compelling to try it out, classifying paddy rice in my hometown region

The current work, is a study case using TW-DTW for classifying paddy rice crops in southern Brazil on `r newthought("__Google Earth Engine__")` platform. All the code used and some further explanation is available on [github](""https://github.com/emanuel-gf/tw-dtw-rice-classification/blob/main"). I also implemented a panel of [GEE](https://ee-emanuelgoulartf.projects.earthengine.app/view/twdtw-resultspanel) to display the results and also the multi-temporal composition.^[
[GEEPanel](https://ee-emanuelgoulartf.projects.earthengine.app/view/twdtw-resultspanel)
]
...
```{r pheno, fig.margin=TRUE,fig.width=3 ,fig.cap="phenology of irrigated rice in Brazil.",echo=FALSE}
knitr::include_graphics("Plots/icons/pheno.png")

```

`r newthought('Think of')` it as: each land cover class has a distinct phenological cycle that is relevant for the space-time classification, which definitely need to be considered for crop monitoring tasks. Therefore, a good suited remote sensing DTW based algorithm needs to balance shape matching with temporal alignment. 

Thus, @Maulsetal  introduced an improvement of the DTW, by adding a time-weight constraint. The improvement gave tools to accounting seasonality as a factor on it. Which has pushed other developments on the domain, include treating classification as an object-based rather than pixel-based task. 


## Google Earth Engine

The __OG__ implementation of tw-dtw is in @R-base, which I love, but my laptop seems to struggle with it on bigger tasks. Necessity pushes us to explore some alternatives and thanks to big-hearted RS community, there is an implementation of the TW-DTW algorithm in GEE made by Willian Oullette. ^[See Github repository: [Willian Oullette]("https://github.com/wouellette/ee-dynamic-time-warping")]
